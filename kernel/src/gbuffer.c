...
static COLOR32 get8(GBUFFER *self, int x, int y);
static void put8(GBUFFER *self, int x, int y, COLOR32 color);
static void fill_rect8(GBUFFER *self, int x, int y, int w, int h, COLOR32 color);
static void blit8(GBUFFER *self, int src_x, int src_y, int w, int h, GBUFFER *dst, int dst_x, int dst_y, int op);

static struct _GBUFFER_METHOD l_gbuf_method8 = { .get = get8, .put = put8,
    .fill_rect = fill_rect8, .blit = blit8 };
struct _GBUFFER_METHOD *g_gbuf_method8 = &l_gbuf_method8;
...
static COLOR32 get8(GBUFFER *self, int x, int y)
{
    ...
    COLOR8 *buf = (COLOR8 *) self->buf;
    return RGB8_TO_32(buf[y * self->w + x]);
}
...
static void put8(GBUFFER *self, int x, int y, COLOR32 color)
{
    ...
    COLOR8 *buf = (COLOR8 *) self->buf;
    buf[y * self->w + x] = RGB32_TO_8(color);
}
...
static void fill_rect8(GBUFFER *self, int x, int y, int w, int h, COLOR32 color)
{
    ...
    COLOR8 *buf = (COLOR16 *) self->buf;
    buf = &buf[y * self->w + x];
    COLOR8 *buf1 = buf;  // 1行目

    COLOR8 col = RGB32_TO_8(color);
    ...
    for (int line = 1; line < h; line++) {
        memcpy(buf, buf1, w * sizeof (COLOR8));
        buf += self->w;
    }
}

static void blit_src_copy8(GBUFFER *src, int src_x, int src_y, int w, int h,
        GBUFFER *dst, int dst_x, int dst_y);
static void blit_src_invert8(GBUFFER *src, int src_x, int src_y, int w, int h,
        GBUFFER *dst, int dst_x, int dst_y);

static void blit8(GBUFFER *self, int src_x, int src_y, int w, int h,
        GBUFFER *dst, int dst_x, int dst_y, int op)
{
    ...
    switch (op) {
    case OP_SRC_COPY:
        blit_src_copy8(self, src_x, src_y, w, h, dst, dst_x, dst_y);
        break;

    case OP_SRC_INVERT:
        blit_src_invert8(self, src_x, src_y, w, h, dst, dst_x, dst_y);
        break;
    }
}
        ...
        GBUFFER *dst, int dst_x, int dst_y)
{
    ...
    COLOR8 *src_buf = (COLOR8 *) src->buf;
    src_buf = &src_buf[src_y * src->w + src_x];;

    COLOR8 *dst_buf = (COLOR8 *) dst->buf;
    dst_buf = &dst_buf[dst_y * dst->w + dst_x];

    if (dst->color_width == 16) {
        COLOR16 *dst_buf16 = (COLOR16 *) dst->buf;
        dst_buf16 = &dst_buf16[dst_y * dst->w + dst_x];

        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                *dst_buf16 = RGB32_TO_16(RGB8_TO_32(*src_buf));
    ...
    if (src->flags & GBF_FLG_HAS_COLORKEY) {
        COLOR8 colorkey = RGB32_TO_8(src->colorkey);
        ...
        if (src->w == w && src->w == dst->w) {
            if (src == dst) {
                memmove(dst_buf, src_buf, w * h * sizeof (COLOR8));
            } else {
                memcpy(dst_buf, src_buf, w * h * sizeof (COLOR8));
            }
        } else {  // src と dst の幅が違うなら１行ずつコピー
            if (src == dst) {
                if (src_y >= dst_y) {
                    for (int y = 0; y < h; y++) {
                        memmove(dst_buf, src_buf, w * sizeof (COLOR8));
                ...
                for (int y = 0; y < h; y++) {
                    memcpy(dst_buf, src_buf, w * sizeof (COLOR8));
        ...
        GBUFFER *dst, int dst_x, int dst_y)
{
    ...
    if (dst->color_width == 8) {
        COLOR8 *dst_buf8 = (COLOR8 *) dst->buf;
        dst_buf8 = &dst_buf8[dst_y * dst->w + dst_x];

        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                *dst_buf8 = RGB32_TO_8(RGB16_TO_32(*src_buf));
